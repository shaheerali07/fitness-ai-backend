exports.setDietPlan = async (req, res) => {
  ...
  user.findOne({ email: email, password: password }).then(async (result) => {
    ...
    await diet.find({ userid: result._id }).then((response) => {
    ...
    if (addStatus === true) {
      ...
      if (
        updateData.meal.breakfast.length === 0 &&
...
exports.setTargetKcal = async (req, res) => {
  ...
  if (searchResult === null) {
    ...
    setTargetKcal.save().then(() => {
      console.log("success");
      res.send({
        message: "success",
        result: setTargetKcal,
      });
    });
  } else {
    await targetkcalModel.findOneAndUpdate(
      { userid: userlist._id },
      updateData,
      { new: true }
    );
